...
parser.add_option('-g', default='gene_tpm_matrix.csv', help="where to output the gene tpm matrix [default: %default")
parser.add_option('-t', default='transcript_tpm_matrix.csv', help="where to output the transcript tpm matrix [default: %default]")
...
#RE_FPKM=re.compile('FPKM "([\-\+\d\.]+)"')
RE_TPM=re.compile('TPM "([\-\+\d\.]+)"')
...
##            geneIDs[t]="|".join(c) #duct-tape transcript IDs together, disregarding ref_gene_names and things like that
    ...
    with open(opts.legend, 'w') as l_file:
        my_writer=csv.writer(l_file,delimiter='\t')
...
##                        transcriptList.append((g_id, t_id, int(ceil(coverage*transcript_len/read_len))))
                    ...
                    t_dict[t_id].setdefault(s[0], tpm)
...
with open(opts.t, 'w') as txtfile:
   txtfile.write("\t".join(["transcript_id"] + [x for x, y in samples]) + "\n")
   for i in t_dict:
        row = [i] + [str(t_dict[i][x]) for x, y in samples]
        txtfile.write("\t".join(row) + "\n")

if opts.v:
   print("..writing %s " % (opts.g))
with open(opts.g, 'w') as txtfile:
   txtfile.write("\t".join(["gene_id"] + [x for x, y in samples]) + "\n")
   for i in geneDict:
        row = [i] + [str(geneDict[i][x]) for x, y in samples]
        txtfile.write("\t".join(row) + "\n")
